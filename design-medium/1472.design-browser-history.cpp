...
public:
  BrowserHistory(string homepage) : history(1, homepage), gap(0) {}
  ...
  string back(int steps) {
    ...
    return gap >= steps ? history[gap -= steps] : history[gap = 0];
  }

  string forward(int steps) {

    return gap + steps >= history.size() ? history[gap = history.size() - 1]
                                         : history[gap += steps];
  }
};
